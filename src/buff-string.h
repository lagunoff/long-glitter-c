...
// int buff_string_diff(struct buff_string_iter *min, struct buff_string_iter *sub);
...
#define BUFF_STRING_BEGIN_INIT(str) {(str)->first && (str)->first->start==0 ? (str)->first->bytes : (str)->bytes, (str), (str)->first, NULL}
